...
<link href="/resource/cmm/js/bootstrap-datepicker/css/datepicker.css" rel="stylesheet" />
<link href="/resource/cmm/js/bootstrap-datepicker/css/datepicker3.css" rel="stylesheet" />
<script src="/resource/cmm/js/bootstrap-datepicker/js/bootstrap-datepicker.js"></script>
<script src="/resource/cmm/js/bootstrap-datepicker/js/locales/bootstrap-datepicker.kr.js" charset="UTF-8"></script>

<link rel="stylesheet" href="/resource/cmm/js/summernote-0.8.11/summernote-lite.css">
<script src="/resource/cmm/js/summernote-0.8.11/summernote-lite.js"></script>
<script src="/resource/cmm/js/summernote-0.8.11/lang/summernote-ko-KR.js"></script>
	...
	jQuery('#frm').validate({
		submitHandler: function(form) {
			...
			if(confirm("저장 하시겠습니까?")) {
				...
				f.action = '/bbs/' + actionUrl + '.do?key=' + menuKey;
				f.submit();
			}
		}
	});
			...
			<tbody>
					...
					<tr>
						...
						<td>
							<label class="radio-inline">
								<input name="noticeAt" title="일반글" id="noticeAtN" type="radio" value="N" <c:if test="${ item.noticeAt eq 'N' || empty item.noticeAt }">checked</c:if> data-parsley-ui-enabled="false" />
								일반글
							</label>
							<label class="radio-inline">
								<input name="noticeAt" title="공지글" id="noticeAtY" type="radio" value="Y" <c:if test="${ item.noticeAt eq 'Y' }">checked</c:if> data-parsley-ui-enabled="false" />
								공지글
							</label>
						</td>
					</tr>
				...
				<c:if test="${ bbsEstbs.secretAt eq 'Y' && bbsEstbs.secretAtAlways eq 'N' }">
					<tr>
						...
						<td>
							<label>
								<input type="checkbox" title="선택 시 비밀글" name="secretAt" value="Y" <c:if test="${ item.secretAt eq 'Y' }">checked</c:if> />
								선택 시 비밀글
							</label>
						</td>
					</tr>
				...
				<c:if test="${ empty userSession && empty item.registPw }">
					<tr>
						...
						<td>
							<input type="password" title="비밀번호를 입력하세요." id="registPw" name="registPw" class="form-control" value="" placeholder="비밀번호 입력" data-parsley-required="true" style="width:50%;" />
						</td>
					</tr>
				</c:if>
							...
							<c:choose>
								...
								<c:when test="${ list.bbsFieldCode eq 'bbsCn' }">
									...
									<tr>
										<th class="span3">${ list.bbsFieldNm } <c:if test="${ list.fieldEssntlAt eq 'Y' }">*</c:if></th>
										<td>
											<textarea id="${ list.bbsFieldCode }" name="${ list.bbsFieldCode }" style="width: 100%; height:250px;" placeholder="${ list.bbsFieldNm } 입력" title="${ list.bbsFieldNm } 입력" <c:if test="${ list.fieldEssntlAt eq 'Y' }">data-required="true"</c:if>>${ item[list.bbsFieldCode] }</textarea>
										</td>
									</tr>
								</c:when>
