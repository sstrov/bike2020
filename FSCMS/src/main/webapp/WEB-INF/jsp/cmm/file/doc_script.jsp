			...
			if(result != null && result.length > 0) {
				...
				for(var i=0; i<result.length; i++) {
					...
					if(array_search(item.atchmnflExtsn.toLowerCase(), imgArray)) {
						...
						imgThumb = '<img src="' + imgThumbSrc + '" alt="" width="100" height="100" />';
					}
					...
					tmpStr += '<tr>' +
										...
										'<td>' +
											...
											'<label><input type="radio" name="fileRepFlag" value="' + item.atchmnflRealNm + '" ' + reprsntThumbAt + ' /> 대표여부</label>' +
										'</td>' +
										'<td>' +
											atchmnflSize +
										'</td>' +
					...
					if(item.fileEtc == 'img' && fileImgContentHtml != '') {
...
function getFileModal() {
	if($('#fileReset_doc').prop('tagName') != undefined) {
		$('#fileReset_doc').hide();
		setPlupload_doc("fileUploader", "/cmm/file/uploadTmp.do?uniqueId=" + fileImgUniqueId + "&limitMegaByte=" + fileImgLimit + "&upKey=" + fileImgUpKey + "&type=file&etcType=file", fileImgLimit);
