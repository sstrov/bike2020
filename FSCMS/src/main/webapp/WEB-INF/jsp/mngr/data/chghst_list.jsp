...
(function($){
	...
	$.getList = function(v) {
		...
		$.ajax({
			url     : "/${ admURI }/data/chghst/getList.do?key=${ param.key }",
			type    : "POST",
			data    : $("#frm").serialize(),
			dataType: "json",
			...
			success:function(result) {
				...
				if(result != null && result.length > 0) {
					for(var i=0; i<result.length; i++) {
						...
						// 체크박스 생성
						var chkBox = '<input type="checkbox" name="chkKey" class="chkKey" value="' + item.dataChghstSn + '" />';
						
						var state = '<small>수정</small>';
						if(item.aftchData == '') {
							state = '<small style="color:red;">삭제</small>';
						} else if(item.bfchgData == '') {
							state = '<small style="color:green;">생성</small>';
						}

						var tableLayout = $("#data-table tfoot").html();
						tableLayout = replaceAll(tableLayout, '{key}',     item.dataChghstSn);
						tableLayout = replaceAll(tableLayout, '{userId}',  (item.mngrId != null)? item.mngrId : item.userId);
						tableLayout = replaceAll(tableLayout, '{tableNm}', item.tableNm);
						...
						tableLayout = replaceAll(tableLayout, '{state}',   (item.bfchgData != item.aftchData)? '<span style="color:red; font-weight:bold;">변경됨</span> (' + state + ')' : '<span style="color:green;">동일</span>');
...
function goView(key) {
	...
	f.dataChghstSn.value = key;
	f.action = "/${ admURI }/data/chghst/view.do?key=${ param.key }";
	f.submit();
}
</script>
</head>
...
<form:form id="frm" name="frm" method="post" commandName="searchVO" cssClass="form-horizontal">
	<input type="hidden" name="dataChghstSn" />
						...
						<div class="col-md-9">
							<label class="select" style="float:left; margin:0;">
								<form:select path="sc" cssClass="form-control">
									...
									<form:option value="TABLE_NM">테이블 명</form:option>
				...
				<div class="panel-body">
					...
					<div class="table-responsive">
						<table id="data-table" class="table table-striped table-bordered">
							...
							<tfoot class="hide">
								<tr>
									<td><span onclick="goView('{key}');" style="cursor:pointer;">{tableNm}</span></td>
									...
									<td><span onclick="goView('{key}');" style="cursor:pointer;">{state}</span></td>
