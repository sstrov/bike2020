...
(function($){
	...
	$.ed_jstree.settings({
		"jstreeObj" : "#jstree",
		"urlList"   : "/${ admURI }/user/menu/getList.do?key=${ param.key }",
		"orderFlag" : orderFlag
	});
	...
	<c:if test="${ !empty role && role.powW eq 'Y' }">
		...
		$.createNode = function(targetKey, node, nodeName, data) {
			...
			$.ajax({
				url     : "/${ admURI }/user/menu/setInsert.do?key=${ param.key }",
				type    : "POST",
				data    : {
					'menuSn' : targetKey,
					'menuNm' : nodeName
		...
		$.renameNode = function(nodeKey, nodeName) {
			...
			$.ajax({
				url     : "/${ admURI }/user/menu/setName.do?key=${ param.key }",
				type    : "POST",
				data    : {
					'menuSn' : nodeKey,
					'menuNm' : nodeName
		...
		$.moveNode =  function(nodeKey, pKey, order) {
			...
			$.ajax({
				url     : "/${ admURI }/user/menu/setOrder.do?key=${ param.key }",
				type    : "POST",
				data    : {
					'menuSn'     : nodeKey,
					'menuUpperSn': pKey,
		...
		$.deleteAll = function(ref, node, nodeKey) {
			...
			// 메뉴 삭제 하위 정보도 삭제
			$.ajax({
				url     : "/${ admURI }/user/menu/deletion.do?key=${ param.key }",
				type    : "POST",
				data    : {
					'menuSn' : nodeKey
				},
	...
	$.treeClick = function() {
		...
		// 선택된 관리자 메뉴 조회
		$.ajax({
			url  : "/${ admURI }/user/menu/getObj.do?key=${ param.key }",
			type : "POST",
			data : {
				"menuSn" : nodeKey
			},
			...
			success:function(result) {
				...
				if(result != null) {
					...
					$("#menuTag").val(unhtmlspecialchars(result.menuTag));
					$("#menuClass").val(result.menuClass);
					...
					if(result.menuLink != null) {
						...
						$("#realUrl").html('/sub.do?key=' + result.menuSn + '<br/><a href="/sub.do?key=' + result.menuSn + '" class="btn btn-primary btn-sm"><i class="fa fa-link"></i> 링크 바로가기</a>');
					}
					
					if(result.menuFileImage != null && result.menuFileImage.length > 0) {
						$("#skinImg").html("<img src=\"" + result.menuFlpth + "/" + result.menuFileImage + "\" width=\"100%\" alt=\"\" /><button type=\"button\" class=\"btn btn-primary btn-sm m-l-5\" onclick=\"del_imgFile();\">이미지 삭제</button>");
					} else {
						$("#skinImg").html("");
					}
	...
	$("#frm").validate({
		submitHandler: function(form) {
			...
			if(confirm("저장 하시겠습니까?")) {
				...
				var menuClass = form.menuClass.value;
				var menuTag   = form.menuTag.value;
				...
				$.ajax({
					url     : "/${ admURI }/user/menu/setInfo.do?key=${ param.key }",
					type    : "POST",
					data    : {
						'menuSn'      : key,
						'accSn'       : accKey,
...
function __setFileData(f) {
	if(confirm("파일을 저장 하시겠습니까?")) {
		...
		f.action = "/cmm/file/imgUpload.do?key=${ param.key }";
...
var uploadHandler = function(uploadFile) {
	...
	$.ajax({
		url     : "/${ admURI }/user/menu/setFile.do?key=${ param.key }",
		type    : "POST",
		data    : {
			'menuSn'        : $('#menuSn').val(),
			'menuFileImage' : uploadFile.serverFileName,
			'menuFlpth'     : uploadFile.relativePath
		},
		dataType : "json",
		error   : function(request, status, error) {
			alert("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
...
function setSiteRecycle() {
	if(confirm("사이트 배치를 진행 하겠습니까?")) {
		...
		$.ajax({
			url     : "/${ admURI }/user/menu/setBatch.do?key=${ param.key }",
			type    : "POST",
			dataType : "json",
			error   : function(request, status, error) {
				alert("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
	...
	<div id="editor_panel">
		...
		<div id="form-panel">
			<ul class="nav nav-tabs">
				...
				<li id="tab_link2" class=""><a href="#tab2" data-toggle="tab">서브 이미지</a></li>
			</ul>
			...
			<div class="tab-content">
				...
				<div class="tab-pane fade in active" id="tab1">
					<form id="frm" name="frm" class="form-horizontal">
						...
						<div class="form-group">
							<label class="col-md-3 control-label">CLASS 명</label>
							<div class="col-md-9 col-sm-9">
								<input type="text" id="menuClass" name="menuClass" class="form-control" <c:if test="${ empty role || role.powW ne 'Y' }">disabled</c:if> placeholder="CLASS 명 입력" maxlength="200" />
							</div>
						</div>
						
						<div class="form-group">
							<label class="col-md-3 control-label">HTML 태그</label>
							<div class="col-md-9 col-sm-9">
								<textarea id="menuTag" name="menuTag" class="form-control" <c:if test="${ empty role || role.powW ne 'Y' }">disabled</c:if> rows="4" placeholder="HTML 태그 입력"></textarea>
							</div>
						</div>
				...
				<div class="tab-pane fade" id="tab2">
					<form id="imgFrm" name="imgFrm" method="post" class="form-horizontal" enctype="multipart/form-data">
						...
						<input type="hidden" name="upKey"       value="menu" />
						
						<div class="form-group">
							<label class="col-md-3 control-label">서브 이미지</label>
							<div class="col-md-9">
