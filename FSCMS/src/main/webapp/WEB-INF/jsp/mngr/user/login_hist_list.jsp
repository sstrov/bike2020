...
(function($){
	...
	$.getList = function(v) {
		...
		$.ajax({
			url     : "/${ admURI }/user/login/hist/getList.do?key=${ param.key }",
			type    : "POST",
			data    : $("#frm").serialize(),
			dataType: "json",
...
function goView(key) {
	...
	f.action = "/${ admURI }/user/login/hist/view.do?key=${ param.key }";
	f.submit();
}
</script>
</head>
						...
						<div class="col-md-9">
							<label class="select" style="float:left; margin:0;">
								<form:select path="sc" cssClass="form-control">
									...
									<form:option value="FS_USER.USER_ID">회원 아이디</form:option>
									<form:option value="FS_USER.USER_NM">회원 명</form:option>
