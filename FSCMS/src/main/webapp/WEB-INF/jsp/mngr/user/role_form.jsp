...
$(document).ready(function() {
	<c:if test="${ !empty role && role.powW eq 'Y' }">
		jQuery('#frm').validate({
			submitHandler: function(form) {
				...
				if(confirm("저장 하시겠습니까?")) {
					...
					f.action = '/${ admURI }/user/role/' + actionUrl + '.do?key=' + menuKey;
					f.submit();
				}
			}
		});
		...
		$("#roleNm").keyup(function() {
			...
			// 아이디 정보 조회
			$.ajax({
				url     : "/${ admURI }/user/role/isExistId.do?key=" + menuKey,
				type    : "POST",
				data    : {
					"roleNm" : this.value
				},
...
// 목록 이동
function goList() {
	f.action = '/${ admURI }/user/role/list.do?key=' + menuKey;
	f.submit();
}
</script>
</head>
						...
						<div class="col-md-6 col-sm-6">
							<table id="data-table" class="table table-striped table-bordered">
								...
								<thead>
									<tr>
										<th>메뉴명</th>
										<th><label><input type="checkbox" <c:if test="${ empty role || role.powW ne 'Y' }">disabled</c:if> onclick="$('.pow').prop('checked', this.checked);" /> 사용권한</label></th>
									</tr>
								</thead>
								<tbody>
									<c:if test="${ !empty contentList }">
										<c:forEach var="content" items="${ contentList }">
											<tr>
												...
												<td><label><input type="checkbox" name="pow" class="pow" value="${ content.menuSn }" <c:if test="${ empty role || role.powW ne 'Y' }">disabled</c:if> <c:if test="${ !empty content.menuAccSn }">checked</c:if> /> 사용권한</label></td>
											</tr>
										</c:forEach>
									</c:if>
								</tbody>
