...
(function($){
	...
	$.getList = function(v) {
		...
		$.ajax({
			url     : "/${ admURI }/banner/getList.do?key=${ param.key }",
			type    : "POST",
			data    : $("#frm").serialize(),
			dataType: "json",
			...
			success:function(result) {
				...
				if(result != null && result.length > 0) {
					for(var i=0; i<result.length; i++) {
						...
						// 체크박스 생성
						var chkBox = '<input type="checkbox" name="chkKey" class="chkKey" value="' + item.bannerSn + '" />';
						
						var date = item.bannerBgnde;
						if(item.bannerEndde != null && item.bannerEndde.length > 0) {
							date += ' ~ ' + item.bannerEndde;
						}
						...
						if(item.useAt == 'N') {
							...
							if(item.bannerBgnde != null && item.bannerBgnde.length > 0) {
								var bannerBgnde = replaceAll(replaceAll(replaceAll(item.bannerBgnde, '-', ''), ':', ''), ' ', '');
								if(bannerBgnde > toDate) {
									state = '<span style="color:green;">준비중</span>';
								} else if(item.bannerEndde != null && item.bannerEndde.length > 0) {
									var bannerEndde = replaceAll(replaceAll(replaceAll(item.bannerEndde, '-', ''), ':', ''), ' ', '');
									if(bannerEndde < toDate) {
						...
						var img = '';
						if(item.flpth != null && item.flpth.length > 0) {
							if(item.atchmnflImage != null && item.atchmnflImage.length > 0) {
								img = '<img src="' + item.flpth + '/200x100_' + item.atchmnflImage + '" alt="" />';
							} else if(item.atchmnflBcrnImage != null && item.atchmnflBcrnImage.length > 0) {
								img = '<img src="' + item.flpth + '/200x100_' + item.atchmnflBcrnImage + '" alt="" />';
							}
						}
						
						var tableLayout = $("#data-table tfoot").html();
						tableLayout = replaceAll(tableLayout, '{key}',    item.bannerSn);
						tableLayout = replaceAll(tableLayout, '{chkBox}', chkBox);
						tableLayout = replaceAll(tableLayout, '{img}',    img);
						tableLayout = replaceAll(tableLayout, '{nm}',     item.bannerNm);
...
function goModify(key) {
	...
	f.bannerSn.value = key;
	f.action = "/${ admURI }/banner/form.do?key=${ param.key }";
	f.submit();
}
</script>
</head>
...
<form:form id="frm" name="frm" method="post" commandName="searchVO" cssClass="form-horizontal">
	<input type="hidden" name="bannerSn" />
	
	<form:hidden path="bannerEstbsSn" />
						...
						<div class="col-md-9">
							<label class="select" style="float:left; margin:0;">
								<form:select path="sc" cssClass="form-control">
									...
									<form:option value="BANNER_NM">팝업존 명</form:option>
								</form:select>
							</label>
				...
				<div class="panel-body">
					...
					<c:if test="${ !empty role.powW && role.powW eq 'Y' }">
						<div style="float:left; width:100%; text-align:right; margin-bottom:10px;">
							<button type="button" class="btn btn-success btn-sm" onclick="goForm('/${ admURI }/banner/form.do?key=${ param.key }');"><i class="fa fa-pencil"></i> 등록</button>
							<button type="button" class="btn btn-purple btn-sm" onclick="goMove('/${ admURI }/banner/move.do?key=${ param.key }');"><i class="fa fa-sort"></i> 순서 변경</button>
							<button type="button" class="btn btn-danger btn-sm" onclick="goDeleteForList('/${ admURI }/banner/deleteForList.do?key=${ param.key }');"><i class="fa fa-trash-o"></i> 선택삭제</button>
						</div>
					</c:if>
						...
						<table id="data-table" data-reorder="true" class="table table-striped table-bordered">
							...
							<tfoot class="hide">
								<tr>
									...
									<td><span onclick="goModify('{key}');" style="cursor:pointer;">{img}</span></td>
									<td><span onclick="goModify('{key}');" style="cursor:pointer;">{nm}</span></td>
