...
function addSubCtgry(ctgrySn){
	
	f.bbsCtgrySn.value = ctgrySn;
	if(ctgrySn != null){
		//alert("not null");
		f.action = '/${ admURI }/bbs/estbs/ctgryForm.do?key=' + menuKey + '&ctgrySn=' + ctgrySn ;
		f.submit();
	} else {
		alert("카테고리를 저장 후 이용해 주세요.");
		return false;
	}
										...
										<c:if test="${ !empty cateList }">
											<c:forEach var="cate" items="${ cateList }" varStatus="idx">
												<tr>
													...
													<td>
														<button type="button" class="btn btn-default btn-sm" onclick="addSubCtgry('${ cate.bbsCtgrySn }');"><i class="fa fa-plus"></i> 추가</button>
													</td>
												</tr>
											</c:forEach>
										</c:if>
