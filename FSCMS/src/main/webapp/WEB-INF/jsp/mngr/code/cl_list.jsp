...
(function($){
	...
	$.getList = function(v) {
		...
		$.ajax({
			url     : "/${ admURI }/code/cl/getList.do?key=${ param.key }",
			type    : "POST",
			data    : $("#frm").serialize(),
			dataType: "json",
			...
			success:function(result) {
				...
				if(result != null && result.length > 0) {
					for(var i=0; i<result.length; i++) {
						...
						// 체크박스 생성
						var chkBox = '<input type="checkbox" name="chkKey" class="chkKey" value="' + item.codeClId + '" />';

						var tableLayout = $("#data-table tfoot").html();
						tableLayout = replaceAll(tableLayout, '{key}',      item.codeClId);
						tableLayout = replaceAll(tableLayout, '{chkBox}',   chkBox);
						tableLayout = replaceAll(tableLayout, '{codeClId}', item.codeClId);
...
function goModify(key) {
	...
	f.codeClId.value = key;
	f.action = "/${ admURI }/code/cl/form.do?key=${ param.key }";
	f.submit();
}
</script>
</head>
...
<form:form id="frm" name="frm" method="post" commandName="searchVO" cssClass="form-horizontal">
	<input type="hidden" name="codeClId" />
						...
						<div class="col-md-9">
							<label class="select" style="float:left; margin:0;">
								<form:select path="sc" cssClass="form-control">
									...
									<form:option value="CODE_CL_ID">분류코드</form:option>
									<form:option value="CODE_CL_NM">분류코드 명</form:option>
									<form:option value="CODE_CL_DC">분류코드 설명</form:option>
								</form:select>
							</label>
				...
				<div class="panel-body">
					...
					<c:if test="${ !empty role.powW && role.powW eq 'Y' }">
						<div style="float:left; width:100%; text-align:right; margin-bottom:10px;">
							<button type="button" class="btn btn-success btn-sm" onclick="goForm('/${ admURI }/code/cl/form.do?key=${ param.key }');"><i class="fa fa-pencil"></i> 등록</button>
							<button type="button" class="btn btn-danger btn-sm" onclick="goDeleteForList('/${ admURI }/code/cl/deleteForList.do?key=${ param.key }');"><i class="fa fa-trash-o"></i> 선택사용안함</button>
						</div>
					</c:if>
						...
						<table id="data-table" class="table table-striped table-bordered" summary="" role="grid" aria-describedby="data-table_info">
							...
							<tfoot class="hide">
								<tr>
									...
									<td><span onclick="goModify('{key}');" style="cursor:pointer;">{codeClId}</span></td>
