...
(function($){
	...
	$.getList = function(v) {
		...
		$.ajax({
			url     : "/${ admURI }/contest/getList.do?key=${ param.key }",
			type    : "POST",
			data    : $("#frm").serialize(),
			dataType: "json",
			...
			success:function(result) {
				...
				if(result != null && result.length > 0) {
					for(var i=0; i<result.length; i++) {
						...
						// 체크박스 생성
						var chkBox = '<input type="checkbox" name="chkKey" class="chkKey" value="' + item.contestSn + '" />';
						var contestEstbsNm = '${item.contestEstbsNm}';

						var tableLayout = $("#data-table tfoot").html();
						tableLayout = replaceAll(tableLayout, '{key}',        item.contestSn);
						tableLayout = replaceAll(tableLayout, '{chkBox}',     chkBox);
						tableLayout = replaceAll(tableLayout, '{contestNm}',  contestEstbsNm);
						tableLayout = replaceAll(tableLayout, '{type}',       item.contestType);
						tableLayout = replaceAll(tableLayout, '{title}',      item.contestNm);
...
function goModify(key) {
	...
	f.contestSn.value = key;
	f.action = "/${ admURI }/contest/form.do?key=${ param.key }";
	f.submit();
}
...
function goList() {
	...
	f.action = "/${ admURI }/contest/estbs/list.do?key=${ param.key }";
	f.submit();
}
...
<form:form id="frm" name="frm" method="post" commandName="searchVO" cssClass="form-horizontal">
	<input type="hidden" name="contestSn" />
	<input type="hidden" name="contestEstbsSn" value="${ item.contestEstbsSn }" />
						...
						<div class="col-md-9">
							<label class="select" style="float:left; margin:0;">
								<form:select path="sc" cssClass="form-control">
									...
									<form:option value="CONTEST_NM">제목</form:option>
									<form:option value="CONTEST_CN">내용</form:option>
									<form:option value="REGIST_NM">작성자</form:option>
								</form:select>
							</label>
				...
				<div class="panel-body">
					...
					<c:if test="${ !empty role.powW && role.powW eq 'Y' }">
						<div style="float:left; width:100%; text-align:right; margin-bottom:10px;">
							<button type="button" class="btn btn-default btn-sm" onclick="goList();">공모전 목록</button>
							
							<button type="button" class="btn btn-success btn-sm" onclick="goForm('/${ admURI }/contest/form.do?key=${ param.key }');"><i class="fa fa-pencil"></i> 등록</button>
							<button type="button" class="btn btn-danger btn-sm" onclick="goDeleteForList('/${ admURI }/contest/deleteForList.do?key=${ param.key }');"><i class="fa fa-trash-o"></i> 선택삭제</button>
						</div>
					</c:if>
						...
						<table id="data-table" class="table table-striped table-bordered">
							...
							<thead>
								<tr>
									...
									<th>제목</th>
									<th>참가자</th>
									<th id="REGIST_DE"><i class="fa fa-fw fa-calendar txt-color-blue hidden-md hidden-sm hidden-xs"></i>참가일</th>
								</tr>
							</thead>
							...
							<tfoot class="hide">
								<tr>
									...
									<td><span onclick="goModify('{key}');" style="cursor:pointer;">{contestNm}</span></td>
									<td><span onclick="goModify('{key}');" style="cursor:pointer;">{type}</span></td>
									<td><span onclick="goModify('{key}');" style="cursor:pointer;">{title}</span></td>
