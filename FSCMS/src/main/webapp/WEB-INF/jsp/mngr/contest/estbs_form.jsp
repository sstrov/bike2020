...
<c:if test="${ !empty item.contestEstbsSn }">
	actionUrl = 'update';
</c:if>
		...
		jQuery('#frm').validate({
			submitHandler: function(form) {
				...
				if(confirm("저장 하시겠습니까?")) {
					...
					f.action = '/${ admURI }/contest/estbs/' + actionUrl + '.do?key=' + menuKey;
					f.submit();
				}
			}
		});
...
// 목록 이동
function goList() {
	f.action = '/${ admURI }/contest/estbs/list.do?key=' + menuKey;
	f.submit();
}
</script>
</head>
				...
				<div class="panel-body panel-form">
					<div class="form-group">
						...
						<div class="col-md-6 col-sm-6">
							<input type="text" id="contestEstbsNm" name="contestEstbsNm" class="form-control" <c:if test="${ empty role || role.powW ne 'Y' }">disabled</c:if> data-parsley-required="true" placeholder="공모전 명 입력" value="${ item.contestEstbsNm }" />
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-md-6 col-sm-6">
							<textarea id="contestEstbsCn" name="contestEstbsCn" class="form-control" <c:if test="${ empty role || role.powW ne 'Y' }">disabled</c:if> placeholder="공모전 내용 입력" style="height:150px;">${ item.contestEstbsCn }</textarea>
						</div>
					</div>
