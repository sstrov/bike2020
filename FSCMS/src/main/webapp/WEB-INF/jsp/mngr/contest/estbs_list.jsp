...
(function($){
	...
	$.getList = function(v) {
		...
		$.ajax({
			url     : "/${ admURI }/contest/estbs/getList.do?key=${ param.key }",
			type    : "POST",
			data    : $("#frm").serialize(),
			dataType: "json",
			...
			success:function(result) {
				...
				if(result != null && result.length > 0) {
					for(var i=0; i<result.length; i++) {
						...
						// 체크박스 생성
						var chkBox = '<input type="checkbox" name="chkKey" class="chkKey" value="' + item.contestEstbsSn + '" />';
	
						var date = item.contestBgnde;
						if(item.contestEndde != null && item.contestEndde.length > 0) {
							date += ' ~ ' + item.contestEndde;
						}
						
						var state = '<span style="color:blue">진행중</span>';
						if(date != null && date.length > 0) {
							if(item.contestBgnde != null && item.contestBgnde.length > 0) {
								var contestBgnde = replaceAll(replaceAll(replaceAll(item.contestBgnde, '-', ''), ':', ''), ' ', '');
								if(contestBgnde > toDate) {
									state = '<span style="color:green;">준비중</span>';
								} else if(item.contestEndde != null && item.contestEndde.length > 0) {
									var contestEndde = replaceAll(replaceAll(replaceAll(item.contestEndde, '-', ''), ':', ''), ' ', '');
									if(contestEndde < toDate) {
										state = '<span style="color:#ccc;font-weight:bold;">마감</span>';
									}
								}
							}
						}
						...
						tableLayout = replaceAll(tableLayout, '{key}',              item.contestEstbsSn);
						tableLayout = replaceAll(tableLayout, '{chkBox}',           chkBox);
						tableLayout = replaceAll(tableLayout, '{contestEstbsNm}',   item.contestEstbsNm);
						tableLayout = replaceAll(tableLayout, '{conDate}',          item.contestBgnde + ' ~ ' + item.contestEndde);
						tableLayout = replaceAll(tableLayout, '{state}',            state);
						tableLayout = replaceAll(tableLayout, '{wCount}',           item.workCount);
						tableLayout = replaceAll(tableLayout, '{wNm}',              item.registNm);
...
function goModify(key) {
	...
	f.contestEstbsSn.value = key;
	f.action = "/${ admURI }/contest/estbs/form.do?key=${ param.key }";
	f.submit();
}

function goExcel(key) {
	var f = document.forms['frm'];
	
	f.contestEstbsSn.value = key;
	f.action = "/${ admURI }/contest/estbs/excel.do?key=${ param.key }";
	f.submit();
}

function goContestList(key) {
	var f = document.forms['frm'];
	
	f.contestEstbsSn.value = key;
	f.action = "/${ admURI }/contest/list.do?key=${ param.key }";
	f.submit();
}
</script>
</head>
...
<form:form id="frm" name="frm" method="post" commandName="searchVO" cssClass="form-horizontal">
	<input type="hidden" name="contestEstbsSn" />
						...
						<div class="col-md-9">
							<label class="select" style="float:left; margin:0;">
								<form:select path="sc" cssClass="form-control">
									...
									<form:option value="FS_CONTEST_ESTBS.CONTEST_ESTBS_NM">공모전 제목</form:option>
									<form:option value="FS_CONTEST_ESTBS.CONTEST_ESTBS_CN">공모전 내용</form:option>
								</form:select>
							</label>
				...
				<div class="panel-body">
					...
					<c:if test="${ !empty role.powW && role.powW eq 'Y' }">
						<div style="float:left; width:100%; text-align:right; margin-bottom:10px;">
							<button type="button" class="btn btn-success btn-sm" onclick="goForm('/${ admURI }/contest/estbs/form.do?key=${ param.key }');"><i class="fa fa-pencil"></i> 등록</button>
							<button type="button" class="btn btn-danger btn-sm" onclick="goDeleteForList('/${ admURI }/contest/estbs/deleteForList.do?key=${ param.key }');"><i class="fa fa-trash-o"></i> 선택삭제</button>
						</div>
					</c:if>
						...
						<table id="data-table" class="table table-striped table-bordered">
							...
							<thead>
								<tr>
									...
									<th>제목</th>
									<th>글쓴이</th>
									<th id="CONTEST_BGNDE"><i class="fa fa-fw fa-calendar txt-color-blue hidden-md hidden-sm hidden-xs"></i>기간</th>
									<th>참여상태</th>
									<th>참여자</th>
									<th>비고</th>
								</tr>
							...
							<tfoot class="hide">
								<tr>
									...
									<td><span onclick="goModify('{key}');" style="cursor:pointer;">{contestEstbsNm}</span></td>
									<td><span onclick="goModify('{key}');" style="cursor:pointer;">{wNm}</span></td>
									<td><span onclick="goModify('{key}');" style="cursor:pointer;">{conDate}</span></td>
									<td><span onclick="goModify('{key}');" style="cursor:pointer;">{state}</span></td>
									<td><span onclick="goContestList('{key}');" style="cursor:pointer;">{wCount}
										&nbsp;<button type="button" class="btn btn-xs btn-purple">작품보기</button></span>
									</td>
									<td><span onclick="goExcel('{key}');" style="cursor:pointer;">
										<button type="button" class="btn btn-xs btn-green">목록다운</button></span>
									</td>
								</tr>
							</tfoot>
						</table>
