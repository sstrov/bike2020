...
(function($){
	...
	$.getList = function(v) {
		...
		$.ajax({
			url     : "/${ admURI }/error/hist/getList.do?key=${ param.key }",
			type    : "POST",
			data    : $("#frm").serialize(),
			dataType: "json",
			...
			success:function(result) {
				...
				if(result != null && result.length > 0) {
					for(var i=0; i<result.length; i++) {
						...
						tableLayout = replaceAll(tableLayout, '{key}',   item.errorHistSn);
						tableLayout = replaceAll(tableLayout, '{msg}',   item.errorMssage);
						tableLayout = replaceAll(tableLayout, '{url}',   item.errorItnadr);
					...
					tmpStr = '<tr class="odd">' +
							'<td colspan="4" align="center" class="dataTables_empty" style="line-height:100px;">등록된 정보가 없습니다.</td>' +
						'</tr>';
...
function goView(key) {
	...
	f.errorHistSn.value = key;
	f.action = "/${ admURI }/error/hist/view.do?key=${ param.key }";
	f.submit();
}
</script>
</head>
...
<form:form id="frm" name="frm" method="post" commandName="searchVO" cssClass="form-horizontal">
	<input type="hidden" name="errorHistSn" />
						...
						<div class="col-md-9">
							<label class="select" style="float:left; margin:0;">
								<form:select path="sc" cssClass="form-control">
									...
									<form:option value="ERROR_MSSAGE">메시지</form:option>
									<form:option value="ERROR_CN">내용</form:option>
				...
				<div class="panel-body">
					...
					<div class="table-responsive">
						<table id="data-table" class="table table-striped table-bordered" summary="" role="grid" aria-describedby="data-table_info">
							...
							<tfoot class="hide">
								<tr>
									<td><span onclick="goView('{key}');" style="cursor:pointer;">{msg}</span></td>
									<td><span onclick="goView('{key}');" style="cursor:pointer;">{url}</span></td>
