...
(function($){
	...
	$.getList = function(v) {
		...
		$.ajax({
			url     : "/${ admURI }/user/getList.do?key=${ param.key }",
			type    : "POST",
			data    : $("#frm").serialize(),
			dataType: "json",
			...
			success:function(result) {
				...
				if(result != null && result.length > 0) {
					for(var i=0; i<result.length; i++) {
						...
						// 체크박스 생성
						var chkBox = '<input type="checkbox" name="chkKey" class="chkKey" value="' + item.userSn + '" />';
						var user = item.userNm + ' (' + item.userId +')';
						var push = '';
						if(item.userMbtlnumRecptnAt == 'Y' && item.userEmailRecptnAt == 'Y'){
							push = '모두';
						}else if(item.userEmailRecptnAt == 'Y'){
							push = '이메일';
						}else if(item.userMbtlnumRecptnAt == 'Y'){
							push = 'SMS';
						}

						var tableLayout = $("#data-table tfoot").html();
						tableLayout = replaceAll(tableLayout, '{key}',      item.userSn);
						tableLayout = replaceAll(tableLayout, '{chkBox}',   chkBox);
						tableLayout = replaceAll(tableLayout, '{user}',     user);
						tableLayout = replaceAll(tableLayout, '{push}',     push);
						tableLayout = replaceAll(tableLayout, '{userId}',   item.userId);
						tableLayout = replaceAll(tableLayout, '{userNm}',   item.userNm);
						tableLayout = replaceAll(tableLayout, '{tel}',      item.userTelno);
						tableLayout = replaceAll(tableLayout, '{email}',    item.userEmail);
						tableLayout = replaceAll(tableLayout, '{noteSn}',   item.noteSn);
						tableLayout = replaceAll(tableLayout, '{expireDe}', item.expireDe);
						tableLayout = replaceAll(tableLayout, '{sponseAt}', (item.lockCo >= 5)? '<strong style="color:red;">잠김</strong>' : '실패횟수(' + item.lockCo + ')');
...
function goModify(key) {
	...
	f.userSn.value = key;
	f.action = "/${ admURI }/user/form.do?key=${ param.key }";
	f.submit();
}
 ...
 * 선택된 내용 삭제
 */
function goPushList(url) {
	if($("input:checkbox[class='chkKey']:checked").length == 0) {
		alert("전송할 회원을 1개이상 선택해 주세요.");
		return;
	}
...
<form:form id="frm" name="frm" method="post" commandName="searchVO" cssClass="form-horizontal">
	<input type="hidden" name="userSn" />
	<input type="hidden" name="sc_expire" />
	<input type="hidden" name="sc_push" value="all"/>
	<input type="hidden" name="userSttus" value="1"/>
				...
				<div class="panel-body">
					...
					<div class="form-group">
						<div class="col-md-10" style="text-align:center;">
							<button type="button" class="btn btn-sm btn-primary m-r-5" onclick="changeSearch('ALL');"><i class="fa fa-search"></i> 전체</button>
							<button type="button" class="btn btn-sm btn-primary m-r-5" onclick="changeSearch('ALL');"><i class="fa fa-search"></i> 모두</button>
							<button type="button" class="btn btn-sm btn-primary m-r-5" onclick="changeSearch('MAIL');"><i class="fa fa-search"></i> 이메일</button>
							<button type="button" class="btn btn-sm btn-primary m-r-5" onclick="changeSearch('SMS');"><i class="fa fa-search"></i> SMS</button>
						</div>
					</div>
					<div class="form-group">
						<div class="col-md-10" style="text-align:center;">
				...
				<div class="panel-body">
					...
					<c:if test="${ !empty role.powW && role.powW eq 'Y' }">
						<div style="float:left; width:100%; text-align:right; margin-bottom:10px;">
							<button type="button" class="btn btn-success btn-sm" onclick="goPushList('/${ admURI }/push/send.do?key=${ param.key }');"><i class="fa fa-pencil"></i> 발송</button>
							<%-- <button type="button" class="btn btn-success btn-sm" onclick="goForm('/${ admURI }/user/form.do?key=${ param.key }');"><i class="fa fa-pencil"></i> 등록</button>
							<button type="button" class="btn btn-danger btn-sm" onclick="goDeleteForList('/${ admURI }/user/deleteForList.do?key=${ param.key }');"><i class="fa fa-trash-o"></i> 선택삭제</button> --%>
						</div>
					</c:if>
