...
(function($){
	...
	$.getList = function(v) {
		...
		$.ajax({
			url     : "/${ admURI }/push/getList.do?key=${ param.key }",
			type    : "POST",
			data    : $("#frm").serialize(),
			dataType: "json",
			...
			success:function(result) {
				...
				if(result != null && result.length > 0) {
					for(var i=0; i<result.length; i++) {
						...
						if(item.pushType == 'S'){
							pushType = 'SMS';
						}else if(item.pushType == 'M'){
							pushType = '이메일';
						}

						var tableLayout = $("#data-table tfoot").html();
						tableLayout = replaceAll(tableLayout, '{key}',      item.pushSn);
						tableLayout = replaceAll(tableLayout, '{chkBox}',   chkBox);
						tableLayout = replaceAll(tableLayout, '{pushType}', pushType);
						tableLayout = replaceAll(tableLayout, '{pushSj}',   item.pushSj);
						tableLayout = replaceAll(tableLayout, '{pushCn}',   item.pushCn);
...
function goView(key) {
	...
	f.pushSn.value = key;
	f.action = "/${ admURI }/push/view.do?key=${ param.key }";
	f.submit();
}
</script>
</head>
						...
						<div class="col-md-9">
							<label class="select" style="float:left; margin:0;">
								<form:select path="sc" cssClass="form-control">
									...
									<form:option value="PUSH_SJ">제목</form:option>
									<form:option value="PUSH_CN">내용</form:option>
								</form:select>
							</label>
				...
				<div class="panel-body">
					...
					<c:if test="${ !empty role.powW && role.powW eq 'Y' }">
						<div style="float:left; width:100%; text-align:right; margin-bottom:10px;">
							<button type="button" class="btn btn-success btn-sm" onclick="goForm('/${ admURI }/push/form.do?key=${ param.key }');"><i class="fa fa-pencil"></i> 등록</button>
							<button type="button" class="btn btn-danger btn-sm" onclick="goDeleteForList('/${ admURI }/push/deleteForList.do?key=${ param.key }');"><i class="fa fa-trash-o"></i> 선택삭제</button>
						</div>
					</c:if>
						...
						<table id="data-table" class="table table-striped table-bordered">
							...
							<tfoot class="hide">
								<tr>
									...
									<td><span onclick="goView('{key}');" style="cursor:pointer;">{pushSj}</span></td>
									<td><span onclick="goView('{key}');" style="cursor:pointer;">{wNm}</span></td>
									<td><span onclick="goView('{key}');" style="cursor:pointer;">{pushType}</span></td>
