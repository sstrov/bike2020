 ...
 */
	...
	$.jstree.reference = function (needle) {
		...
		if(!obj || !obj.length) {
			try { obj = $(needle); } catch (ignore) { console.log('jstree : ' + ignore); }
		}
		if(!obj || !obj.length) {
			try { obj = $('#' + needle.replace($.jstree.idregex,'\\$&')); } catch (ignore) { console.log('jstree : ' + ignore); }
		}
						...
						if(document.selection && document.selection.empty) {
							...
							if(window.getSelection) {
								...
								try {
									sel.removeAllRanges();
									sel.collapse();
								} catch (ignore) { console.log('jstree : ' + ignore); }
							}
						}
					})
	...
	$.jstree.plugins.search = function (options, parent) {
		...
		this._search_open = function (d) {
			...
			$.each(d.concat([]), function (i, v) {
				if(v === "#") { return true; }
				try { v = $('#' + v.replace($.jstree.idregex,'\\$&'), t.element); } catch(ignore) { console.log('jstree : ' + ignore); }
				if(v && v.length) {
					if(t.is_closed(v)) {
