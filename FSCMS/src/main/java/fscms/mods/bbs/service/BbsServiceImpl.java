...
public class BbsServiceImpl extends EgovAbstractServiceImpl implements BbsService {
	...
	@Override
	public void insertInfo(HttpServletRequest request, BbsVO vo, String mngrId, String uniqueId) throws Exception {
		bbsDAO.insertInfo(vo);
		
		dataChghstHelper.setInsert(request, "FS_BBS", null, this.getData(vo.getBbsSn()), null, mngrId);
		...
		fileHelper.insertFile(request, "bbs", "" + vo.getBbsSn(), uniqueId, null, thumbs, thumbNms, mngrId);
	}

	@Override
	public void updateInfo(HttpServletRequest request, String bfData, BbsVO vo, String mngrId, String uniqueId) throws Exception {
		bbsDAO.updateInfo(vo);
		
		if(StringUtils.isNotEmpty(bfData)) {
			dataChghstHelper.setInsert(request, "FS_BBS", bfData, this.getData(vo.getBbsSn()), null, mngrId);
		}
	...
	@Override
	public void updateOrder(HttpServletRequest request, String bfData, BbsVO vo, String mngrId) throws Exception {
	...
	@Override
	public void deleteInfo(HttpServletRequest request, BbsVO vo, String mngrId) throws Exception {
		...
		this.deleteProc(request, vo, mngrId);
	}
	 ...
	 */
	private void deleteChildren(HttpServletRequest request, List<BbsVO> itemList, String mngrId) throws Exception {
		...
		if(itemList != null && itemList.size() > 0) {
			for (BbsVO item : itemList) {
				...
				this.deleteProc(request, item, mngrId);
			}
		}
	}
	 ...
	 * @Method 설명	: 정보 삭제
	 */
	private void deleteProc(HttpServletRequest request, BbsVO bbs, String mngrId) throws Exception {
		...
		if(cmList != null && cmList.size() > 0) {
			for (BbsCmVO cm : cmList) {
				...
				bbsCmService.deleteInfo(request, bfData_cm, cm, mngrId);
			}
		}
		...
		if(fileList != null && fileList.size() > 0) {
			for (AtchmnflVO file : fileList) {
				...
				atchmnflService.deleteInfo(request, bfData_file, file, mngrId);
			}
		}
		...
		dataChghstHelper.setInsert(request, "FS_BBS", bfData, null, null, mngrId);
