...
public class BbsVO implements Serializable {
	...
	public void setLastDayValue(int lastDayValue) {
	...
	/**달력을 출력하기 위해 필요한 값들을 설정하는 메소드*/
	@SuppressWarnings("static-access")
	public void checkSYearAndSMonth(){
		
		Calendar now = Calendar.getInstance();//오늘
		Calendar firstDay = Calendar.getInstance();//달의 첫날
		Calendar lastDay = Calendar.getInstance();//달의 마지막날
		
		if(getsYear()==null){
			setsYear(now.get(Calendar.YEAR));
		}
		if(getsMonth()==null){
			setsMonth(now.get(Calendar.MONTH));
		}else{
			setsMonth(getsMonth()-1);
			if(getsMonth()>11){
				setsMonth(0);
				setsYear(getsYear()+1);
			}else if(getsMonth()<0){
		...
		firstDay.set(sYear, sMonth, 1, 0, 0, 0);
		firstDay.set(Calendar.MILLISECOND, 0);
		
		lastDayValue = firstDay.getActualMaximum(Calendar.DATE);
		
		lastDay.set(sYear, sMonth, lastDayValue, 23, 59, 59);
		lastDay.set(Calendar.MILLISECOND, 0);
		
		firstDayOfWeek = this.getDayOfWeek(firstDay.getTime());
		lastDayOfWeek = this.getDayOfWeek(lastDay.getTime());
		
		//해당 달의 주 최대수
		maxWeeks = (lastDayValue + (firstDayOfWeek - 1) + (7 - lastDayOfWeek))/7;
