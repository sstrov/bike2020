...
public class AtchmnflServiceImpl extends EgovAbstractServiceImpl implements AtchmnflService {
	...
	@Override
	public void insertInfo(HttpServletRequest request, AtchmnflVO vo, String mngrId)
			throws Exception {
		atchmnflDAO.insertInfo(vo);
		
		dataChghstHelper.setInsert(request, "FS_ATCHMNFL", null, this.getData(vo.getAtchmnflSn()), null, mngrId);
	}

	@Override
	public void updateInfo(HttpServletRequest request, String bfData, AtchmnflVO vo, String mngrId)
			throws Exception {
		atchmnflDAO.updateInfo(vo);
		
		if(StringUtils.isNotEmpty(bfData)) {
			dataChghstHelper.setInsert(request, "FS_ATCHMNFL", bfData, this.getData(vo.getAtchmnflSn()), null, mngrId);
		}
	}

	@Override
	public void deleteInfo(HttpServletRequest request, String bfData, AtchmnflVO vo, String mngrId)
			throws Exception {
		atchmnflDAO.deleteInfo(vo);
		
		if(StringUtils.isNotEmpty(bfData)) {
			dataChghstHelper.setInsert(request, "FS_ATCHMNFL", bfData, null, null, mngrId);
		}
	}
