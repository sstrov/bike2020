...
(function($){
	...
	$.getList = function(v) {
		...
		$.ajax({
			url     : "/${ admURI }/banner/estbs/getList.do?key=${ param.key }",
			type    : "POST",
			data    : $("#frm").serialize(),
			dataType: "json",
			...
			success:function(result) {
				...
				if(result != null && result.length > 0) {
					for(var i=0; i<result.length; i++) {
						...
						// 체크박스 생성
						var chkBox = '<input type="checkbox" name="chkKey" class="chkKey" value="' + item.bannerEstbsSn + '" />';
						
						var tableLayout = $("#data-table tfoot").html();
						tableLayout = replaceAll(tableLayout, '{key}',    item.bannerEstbsSn);
						tableLayout = replaceAll(tableLayout, '{chkBox}', chkBox);
						tableLayout = replaceAll(tableLayout, '{nm}',     item.bannerEstbsNm);
						tableLayout = replaceAll(tableLayout, '{size}',   item.widthSize + ((item.vrticlSize != null)? "x" + item.vrticlSize : ""));
						tableLayout = replaceAll(tableLayout, '{upByte}', item.uploadLmttCpcty);
...
// 배너 상세 목록 이동
function goBanner(key) {
	var f = document.forms['frm'];
	
	f.orderBy.value = "";
	f.bannerEstbsSn.value = key;
	f.action = "/${ admURI }/banner/list.do?key=${ param.key }";
	f.submit();
}
...
function goModify(key) {
	...
	f.bannerEstbsSn.value = key;
	f.action = "/${ admURI }/banner/estbs/form.do?key=${ param.key }";
	f.submit();
}
</script>
</head>
...
<form:form id="frm" name="frm" method="post" commandName="searchVO" cssClass="form-horizontal">
	<input type="hidden" name="bannerEstbsSn" />
						...
						<div class="col-md-9">
							<label class="select" style="float:left; margin:0;">
								<form:select path="sc" cssClass="form-control">
									...
									<form:option value="BANNER_ESTBS_NM">팝업존 설정 명</form:option>
									<form:option value="BANNER_ESTBS_DC">팝업존 설명</form:option>
								</form:select>
							</label>
				...
				<div class="panel-body">
					...
					<c:if test="${ !empty role.powW && role.powW eq 'Y' }">
						<div style="float:left; width:100%; text-align:right; margin-bottom:10px;">
							<button type="button" class="btn btn-success btn-sm" onclick="goForm('/${ admURI }/banner/estbs/form.do?key=${ param.key }');"><i class="fa fa-pencil"></i> 등록</button>
							<button type="button" class="btn btn-danger btn-sm" onclick="goDeleteForList('/${ admURI }/banner/estbs/deleteForList.do?key=${ param.key }');"><i class="fa fa-trash-o"></i> 선택삭제</button>
						</div>
					</c:if>
						...
						<table id="data-table" class="table table-striped table-bordered">
							...
							<thead>
								<tr>
									...
									<th id="BANNER_ESTBS_NM">팝업존 설정 명</th>
									<th>노출사이즈</th>
									<th>업로드 용량</th>
							...
							<tfoot class="hide">
								<tr>
									...
									<td><span onclick="goBanner('{key}');" style="cursor:pointer;">{nm}</span></td>
									<td><span onclick="goBanner('{key}');" style="cursor:pointer;">{size}</span></td>
									<td><span onclick="goBanner('{key}');" style="cursor:pointer;">{upByte}</span></td>
									<td><span onclick="goBanner('{key}');" style="cursor:pointer;">{wDate}</span></td>
									<td><button type="button" class="btn btn-xs btn-purple" onclick="goModify('{key}');"><i class="fa fa-cog"></i> 관리</button></td>
								</tr>
							</tfoot>
						</table>
					</div>
