...
(function($){
	...
	$.getList = function(v) {
		...
		$.ajax({
			url     : "/${ admURI }/code/getList.do?key=${ param.key }",
			type    : "POST",
			data    : $("#frm").serialize(),
			dataType: "json",
			...
			success:function(result) {
				...
				if(result != null && result.length > 0) {
					for(var i=0; i<result.length; i++) {
						...
						// 체크박스 생성
						var chkBox = '<input type="checkbox" name="chkKey" class="chkKey" value="' + item.codeId + '" />';

						var tableLayout = $("#data-table tfoot").html();
						tableLayout = replaceAll(tableLayout, '{key}',      item.codeId);
						tableLayout = replaceAll(tableLayout, '{chkBox}',   chkBox);
						tableLayout = replaceAll(tableLayout, '{codeCl}',   item.codeClId);
						tableLayout = replaceAll(tableLayout, '{codeClNm}', item.codeClNm);
						tableLayout = replaceAll(tableLayout, '{codeId}',   item.codeId);
						tableLayout = replaceAll(tableLayout, '{codeIdNm}', item.codeNm);
						tableLayout = replaceAll(tableLayout, '{useAt}',    (item.useAt == 'Y')? '<span style="color:green;">사용</span>' : '<span style="color:red; font-weight:bold;">사용안함</span>');
...
function changeSearch() {
	...
	var sc_useAt = "";
	$("input:checkbox[name='tmp_useAt']:checked").each(function(idx, item) {
		sc_useAt += ((sc_useAt != "" && sc_useAt.length > 0)? "," : "") + $(this).val();
	});
	f.sc_useAt.value = sc_useAt;
...
function goModify(key) {
	...
	f.codeId.value = key;
	f.action = "/${ admURI }/code/form.do?key=${ param.key }";
	f.submit();
}
</script>
</head>
						...
						<div class="col-md-9">
							<label class="select" style="float:left; margin:0;">
								<form:select path="sc" cssClass="form-control">
									...
									<form:option value="CODE_ID">코드 아이디</form:option>
									<form:option value="CODE_CL_ID">분류코드 아이디</form:option>
									<form:option value="CODE_NM">코드 명</form:option>
									<form:option value="CODE_DC">코드 설명</form:option>
								</form:select>
							</label>
					...
					<div class="form-group">
						...
						<div class="col-md-9">
							<label class="checkbox checkbox-inline" style="float:left; margin-right:20px;">
								<input type="checkbox" id="tmp_useAtY" name="tmp_useAt" value="Y" <c:if test="${ fn:indexOf(searchVO.sc_useAt, 'Y') ne -1 }">checked</c:if> />
								사용
							</label>
							<label class="checkbox checkbox-inline">
								<input type="checkbox" id="tmp_useAtN" name="tmp_useAt" value="N" <c:if test="${ fn:indexOf(searchVO.sc_useAt, 'N') ne -1 }">checked</c:if> />
								사용안함
							</label>
						</div>
					</div>
				...
				<div class="panel-body">
					...
					<c:if test="${ !empty role.powW && role.powW eq 'Y' }">
						<div style="float:left; width:100%; text-align:right; margin-bottom:10px;">
							<button type="button" class="btn btn-success btn-sm" onclick="goForm('/${ admURI }/code/form.do?key=${ param.key }');"><i class="fa fa-pencil"></i> 등록</button>
							<button type="button" class="btn btn-danger btn-sm" onclick="goDeleteForList('/${ admURI }/code/deleteForList.do?key=${ param.key }');"><i class="fa fa-trash-o"></i> 선택사용안함</button>
						</div>
					</c:if>
						...
						<table id="data-table" class="table table-striped table-bordered">
							...
							<thead>
								<tr>
									...
									<th id="CODE_CL_ID">분류코드 아이디</th>
									<th>분류코드 명</th>
									<th id="CODE_ID">코드 아이디</th>
									<th id="CODE_NM">코드 명</th>
									<th id="USE_AT">사용여부</th>
							...
							<tfoot class="hide">
								<tr>
									...
									<td><span onclick="goModify('{key}');" style="cursor:pointer;">{codeCl}</span></td>
									<td><span onclick="goModify('{key}');" style="cursor:pointer;">{codeClNm}</span></td>
									<td><span onclick="goModify('{key}');" style="cursor:pointer;">{codeId}</span></td>
									<td><span onclick="goModify('{key}');" style="cursor:pointer;">{codeIdNm}</span></td>
									<td><span onclick="goModify('{key}');" style="cursor:pointer;">{useAt}</span></td>
