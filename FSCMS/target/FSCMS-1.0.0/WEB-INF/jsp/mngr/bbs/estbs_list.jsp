...
(function($){
	...
	$.getList = function(v) {
		...
		$.ajax({
			url     : "/${ admURI }/bbs/estbs/getList.do?key=${ param.key }",
			type    : "POST",
			data    : $("#frm").serialize(),
			dataType: "json",
			...
			success:function(result) {
				...
				if(result != null && result.length > 0) {
					for(var i=0; i<result.length; i++) {
						...
						// 체크박스 생성
						var chkBox = '<input type="checkbox" name="chkKey" class="chkKey" value="' + item.bbsEstbsSn + '" />';

						var tableLayout = $("#data-table tfoot").html();
						tableLayout = replaceAll(tableLayout, '{key}',      item.bbsEstbsSn);
						tableLayout = replaceAll(tableLayout, '{chkBox}',   chkBox);
						tableLayout = replaceAll(tableLayout, '{skin}',     item.sknSnDp1Nm + " > " + item.sknSnDp2Nm);
						tableLayout = replaceAll(tableLayout, '{nm}',       item.bbsEstbsNm);
						tableLayout = replaceAll(tableLayout, '{powList}',  getPowType(item.authorList));
						tableLayout = replaceAll(tableLayout, '{powRead}',  getPowType(item.authorRedng));
						tableLayout = replaceAll(tableLayout, '{powWrite}', getPowType(item.authorRegist));
...
function goModify(key) {
	...
	f.bbsEstbsSn.value = key;
	f.action = "/${ admURI }/bbs/estbs/form.do?key=${ param.key }";
	f.submit();
}
</script>
</head>
...
<form:form id="frm" name="frm" method="post" commandName="searchVO" cssClass="form-horizontal">
	<input type="hidden" name="bbsEstbsSn" />
						...
						<div class="col-md-9">
							<label class="select" style="float:left; margin:0;">
								<form:select path="sc" cssClass="form-control">
									...
									<form:option value="BBS_ESTBS_NM">게시판 명</form:option>
								</form:select>
							</label>
				...
				<div class="panel-body">
					...
					<c:if test="${ !empty role.powW && role.powW eq 'Y' }">
						<div style="float:left; width:100%; text-align:right; margin-bottom:10px;">
							<button type="button" class="btn btn-success btn-sm" onclick="goForm('/${ admURI }/bbs/estbs/form.do?key=${ param.key }');"><i class="fa fa-pencil"></i> 등록</button>
							<button type="button" class="btn btn-danger btn-sm" onclick="goDeleteForList('/${ admURI }/bbs/estbs/deleteForList.do?key=${ param.key }');"><i class="fa fa-trash-o"></i> 선택삭제</button>
						</div>
					</c:if>
						...
						<table id="data-table" class="table table-striped table-bordered" summary="" role="grid" aria-describedby="data-table_info">
							...
							<tfoot class="hide">
								<tr>
									...
									<td><span onclick="goModify('{key}');" style="cursor:pointer;">{skin}</span></td>
									<td><span onclick="goModify('{key}');" style="cursor:pointer;">{nm}</span></td>
									<td><span onclick="goModify('{key}');" style="cursor:pointer;">{powList}</span></td>
									<td><span onclick="goModify('{key}');" style="cursor:pointer;">{powRead}</span></td>
									<td><span onclick="goModify('{key}');" style="cursor:pointer;">{powWrite}</span></td>
								</tr>
							</tfoot>
						</table>
					</div>
