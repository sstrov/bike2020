...
(function($){
	...
	$.getList = function(v) {
		...
		$.ajax({
			url     : "/${ admURI }/mngr/login/hist/getList.do?key=${ param.key }",
			type    : "POST",
			data    : $("#frm").serialize(),
			dataType: "json",
			...
			success:function(result) {
				...
				if(result != null && result.length > 0) {
					for(var i=0; i<result.length; i++) {
						...
						// 체크박스 생성
						var chkBox = '<input type="checkbox" name="chkKey" class="chkKey" value="' + item.loginHistSn + '" />';
						
						var tableLayout = $("#data-table tfoot").html();
						tableLayout = replaceAll(tableLayout, '{key}',    item.loginHistSn);
						tableLayout = replaceAll(tableLayout, '{userId}', item.mngrId);
						tableLayout = replaceAll(tableLayout, '{userNm}', item.mngrNm);
						tableLayout = replaceAll(tableLayout, '{plf}',    item.loginPltfom);
						tableLayout = replaceAll(tableLayout, '{brow}',   item.loginBrwsrNm + ' (' + item.loginBrwsrVer + ')');
						tableLayout = replaceAll(tableLayout, '{ip}',     item.loginIp);
						tableLayout = replaceAll(tableLayout, '{sDate}',  item.loginBgnde);
						tableLayout = replaceAll(tableLayout, '{eDate}',  item.loginEndde);
...
function goView(key) {
	...
	f.loginHistSn.value = key;
	f.action = "/${ admURI }/mngr/login/hist/view.do?key=${ param.key }";
	f.submit();
}
</script>
</head>
...
<form:form id="frm" name="frm" method="post" commandName="searchVO" cssClass="form-horizontal">
	<input type="hidden" name="loginHistSn" />
						...
						<div class="col-md-9">
							<label class="select" style="float:left; margin:0;">
								<form:select path="sc" cssClass="form-control">
									...
									<form:option value="FS_MNGR.MNGR_ID">관리자 아이디</form:option>
									<form:option value="FS_MNGR.MNGR_NM">관리자 명</form:option>
									<form:option value="LOGIN_IP">등록 아이피</form:option>
								</form:select>
							</label>
						...
						<table id="data-table" class="table table-striped table-bordered" summary="" role="grid" aria-describedby="data-table_info">
							...
							<thead>
								<tr>
									...
									<th>등록 브라우저</th>
									<th>등록 아이피</th>
									<th id="LOGIN_BGNDE"><i class="fa fa-fw fa-calendar txt-color-blue hidden-md hidden-sm hidden-xs"></i>로그인 일자</th>
									<th id="LOGIN_ENDDE"><i class="fa fa-fw fa-calendar txt-color-blue hidden-md hidden-sm hidden-xs"></i>로그아웃 일자</th>
								</tr>
							</thead>
							...
							<tfoot class="hide">
								<tr>
									...
									<td><span onclick="goView('{key}');" style="cursor:pointer;">{userNm}</span></td>
									...
									<td><span onclick="goView('{key}');" style="cursor:pointer;">{sDate}</span></td>
									<td><span onclick="goView('{key}');" style="cursor:pointer;">{eDate}</span></td>
								</tr>
							</tfoot>
						</table>
					</div>
